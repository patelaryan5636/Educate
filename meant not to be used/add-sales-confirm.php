...
<head>
    ...
    <meta http-equiv="refresh" content="0; url=makesale.php">
...
<div class="dropdown-menu dropdown-menu-right">
    ...
    <div class="content">
        ...
        <?php
        $total = 0; // Variable to store the total cost of all products
        ...
        foreach ($selectedProducts as $productIdfromArray) {
            ...
            while ($data = mysqli_fetch_array($resultofarray)) {
                ...
                foreach ($_POST as $key => $value) {
                    // Check if the name starts with "price_" to identify the relevant prices
                    if (strpos($key, 'price_') === 0) {
                        // Extract the product ID from the key
                        $productId = substr($key, strlen('price_'));

                        // Check if the product ID matches the current product in the loop
                        if ($productId == $productIdfromArray) {
                            // Process the value of the corresponding price
                            echo $value;

                            // Calculate subtotal for the product
                            $price = floatval($value);
                            $quantityKey = 'textbox_' . $productIdfromArray;
                ...
                foreach ($_POST as $key => $value) {
                    // Check if the name starts with "textbox_" to identify the relevant text boxes
                    if (strpos($key, 'textbox_') === 0) {
                        // Extract the product ID from the key
                        $productId = substr($key, strlen('textbox_'));

                        // Check if the product ID matches the current product in the loop
                        if ($productId == $productIdfromArray) {
                            // Process the value of the corresponding text box
                            echo $value;
                            break;
                        }
                    }
                ...
                echo "</div>";
                echo "<div class='col-lg-2'>";
                echo $subtotal; // Output the subtotal for the product
                echo "</div>";
                echo "</div>";
            }
        }
        ...
        <div class="row">
            ...
            <div class="col-lg-2"></div>
            <div class="col-lg-2">
                <strong><?php echo $total; ?></strong> <!-- Output the total cost of all products -->
            </div>
        </div>
        ...
        <?php
        $orderTax = isset($_POST["taxTextBox"]) ? $_POST["taxTextBox"] : 0;
        $discount = isset($_POST["discountTextBox"]) ? $_POST["discountTextBox"] : 0;
        $grandTotal = 0;
        $subtotal = $total;
        ...
        foreach ($selectedProducts as $productIdfromArray) {
            ...
            while ($data = mysqli_fetch_array($resultofarray)) {
                ...
                foreach ($_POST as $name => $value) {
                    // Check if the name starts with "textbox_" to identify the relevant text boxes
                    if (strpos($name, 'textbox_') === 0) {
                        // Extract the product ID from the key
                        $productId = substr($name, strlen('textbox_'));

                        // Check if the product ID matches the current product in the loop
                        if ($productId == $productIdfromArray) {
                            // Process the value of each text box

                            foreach ($_POST as $price_key => $price) {
                                // Check if the name starts with "price_" to identify the relevant prices
                                if (strpos($price_key, 'price_') === 0) {
                                    // Extract the product ID from the key
                                    $priceProductId = substr($price_key, strlen('price_'));

                                    // Check if the product ID matches the current product in the loop
                                    if ($priceProductId == $productIdfromArray) {
                                        // Process the value of each text box
                                        $originalPrice = $price;
                                        break;
                                    }
            ...
            <div class="col-lg-6">
                <div class="total-order w-100 max-widthauto m-auto mb-4">
                    <ul>
                        <li>
                            <h4>Order Tax</h4>
                            <h5><?php echo number_format($orderTax, 0); ?>%</h5>
                        </li>
                        <li>
                            <h4>Discount</h4>
        ...
        <div class="col-lg-12">
            <a href="makesale.php" class="btn btn-submit me-2">Submit</a>
